...
- `regions.json` — 업체명·전화·텔레그램 링크·도메인·운영시간·지역 목록·갤러리 파일 목록.
  ...
  - `intro`: 지역별 소개 문장. 끝에 붙는 결제 안내 문장("선입금 없이 만나서 결제합니다." 등)은 빌드가 지역마다 다른 표현으로 자동으로 붙이므로 쓰지 않는다(써 두면 자동 문장은 생략). 없으면 `areas` 앞 3개 지명으로 만든 기본 문장을 쓰지만, 주력 지역은 그 지역에만 해당하는 내용으로 직접 쓰는 편이 낫다.
  - 지역 페이지 소개 카드 문단은 `build.py`의 `CARD1_P1` 등 변형 목록 중 하나가 slug 기준으로 고정 선택된다(지역마다 같은 문장이 반복되지 않게). 문구를 고치려면 `template.html`이 아니라 그 목록을 고친다.
