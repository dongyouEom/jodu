...
# 두 문장 모두 지명(areas 앞 3개)을 넣어 같은 패턴을 쓰는 지역끼리도 문장이 겹치지 않게 한다.
# 지명 뒤에 받침에 따라 바뀌는 조사(은/는, 이/가, 을/를)는 쓰지 않는다.
INTRO_FALLBACK = [
    "{a0} 상권과 {a1} 일대까지 전문 관리사가 직접 방문합니다. {a2} 쪽도 이동 동선이 짧아 예약 후 빠르게 도착합니다.",
    "{a0}부터 {a1}까지 {city} 전 지역을 관리사가 직접 찾아갑니다. {a2} 인근에도 대기 중인 관리사가 있어 늦은 시간 배정이 빠른 편입니다.",
    "{a0} 인근과 {a1} 일대 원룸·오피스텔까지 빠짐없이 방문합니다. {a2} 등 {city} 어디든 30분 내외로 도착하도록 관리사를 배정합니다.",
    "{a0}·{a1} 등 {city} 전 지역에 전문 관리사가 방문합니다. {a2}까지도 이동 동선이 짧아 예약 후 대기 시간이 길지 않습니다.",
    "{a0} 생활권과 {a1} 인근까지 {city} 전역을 관리사가 직접 방문합니다. 예약이 몰리는 시간에도 {a2} 방면까지 빠르게 배정해드립니다.",
]
# 수기 intro는 저마다 지역 특징이 담겨 있어 그대로 두되(문장 자체가 이미 고유함),
# 전부 똑같이 끝맺던 마지막 문장만 지역별로 다르게 고른다.
PAYMENT_NOTE = [
    "선입금 없이 만나서 결제합니다.",
    "예약금 없이 만나서 결제하는 방식입니다.",
    "선입금 요구 없이 뵙고 나서 결제합니다.",
    "선입금·예약금 없이 만나서 바로 결제합니다.",
    "예약금 없이 현장에서 만나 결제합니다.",
]
...
for r in regions:
    ...
    a = [*r["areas"], r["city"], r["city"]]  # areas 가 3개 미만이어도 빈 자리는 시·구 이름으로 채운다
    intro = r.get("intro") or pick(r["slug"], "intro", INTRO_FALLBACK).format(
        a0=a[0], a1=a[1], a2=a[2], city=r["city"])
    # intro 에 결제 문장까지 직접 써 둔 경우엔 두 번 붙이지 않는다
    if not re.search(r"결제[^.]*\.?\s*$", intro):
        intro += " " + pick(r["slug"], "payment_note", PAYMENT_NOTE)
