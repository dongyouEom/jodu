...
INTRO_FALLBACK = [
    "{a0} 상권과 {a1} 일대까지 전문 관리사가 직접 방문합니다. {city} 어디든 이동 동선이 짧아 예약 후 빠르게 도착합니다.",
    "{a0}부터 {a1}까지 {city} 전 지역을 관리사가 직접 찾아갑니다. 대기 인원이 많아 늦은 시간에도 배정이 빠른 편입니다.",
    "{a0} 인근과 {a1} 일대 원룸·오피스텔까지 빠짐없이 방문합니다. {city} 어디든 30분 내외로 도착하도록 관리사를 배정합니다.",
    "{a0}·{a1} 등 {city} 전 지역에 전문 관리사가 방문합니다. 이동 동선이 짧아 예약 후 대기 시간이 길지 않습니다.",
    "{a0} 생활권과 {a1} 인근까지 {city} 전역을 관리사가 직접 방문합니다. 예약이 몰리는 시간에도 빠르게 배정해드립니다.",
]
# 118개 수기 intro는 저마다 지역 특징이 담겨 있어 그대로 두되(문장 자체가 이미 고유함),
...
for r in regions:
    ...
    intro_body = r.get("intro") or pick(r["slug"], "intro", INTRO_FALLBACK).format(
        a0=r["areas"][0], a1=r["areas"][1], city=r["city"])
    intro = f'{intro_body} {pick(r["slug"], "payment_note", PAYMENT_NOTE)}'
